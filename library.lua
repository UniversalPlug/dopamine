...
local function ZGJC_fake_script() -- Fake Script: StarterGui.Riftcore.UIHandler
		...
		return {
			...
			CreateDefaultSettings = function(self)
				...
				setting_configs_section:AddButton({
					Text = "Create config",
					Callback = function(state)
						local configName = ConfigInputs.configName and ConfigInputs.configName.TextBox.Text or ""
	...
	local Window = CreateWindow({
		Title = "Dopamine.xyz | Lone Survival",
	...
	local SettingsTab = Window:CreateDefaultSettings()
	
	local main_section = Window:AddSection("Main", "left", MainTab)
	local world_section = Window:AddSection("World", "right", MainTab)
	local misc_section = Window:AddSection("Misc", "right", MainTab)
	
	local players_esp_section = Window:AddSection("Players", "left", VisualsTab)
	local world_esp_section = Window:AddSection("World", "right", VisualsTab)
	
	local silent_aim_section = Window:AddSection("Silent Aim", "left", CombatTab)
	local gun_mods_section = Window:AddSection("Gun Mods", "right", CombatTab)
	local melee_mods_section = Window:AddSection("Melee Mods", "right", CombatTab)
	
	local WorldSection = Window:AddSection("World", "left", WorldTab)
	
	--// Main sections
	main_section:AddToggle({
		Text = "Instant Quick Loot",
		Default = false,
		Callback = function(state)
	...
	main_section:AddSeperator("Sigma")
	...
	silent_aim_section:AddSeperator("Settings")
	
	silent_aim_section:AddDropdown({
		Values = { "Head", "Neck", "Chest", "Torso", "Pelvis", "Left Arm", "Right Arm", "Left Leg", "Right Leg", "Left Foot", "Right Foot" },
		Default = "Head",
		Multi = true,
		Text = 'Target Part',
	...
	players_esp_section:AddSeperator("Settings")
	
	
	local barrelesp = false
	local barrelDropdown
	
	local droppeditemsesp = false
	local droppeditemsDropdown
	
	local cratesesp = false
	...
	world_esp_section:AddToggle({
		Text = 'Barrel ESP',
		Default = false,
		Callback = function(state)
			barrelesp = state
			if barrelDropdown then
				barrelDropdown.Visible = state
			end
		end
	})
	
	barrelDropdown = world_esp_section:AddDropdown({
		Values = { "Red Barrel", "Tan Barrel", "Blue Barrel" }, 
		Default = "",
		Multi = false,
		Text = 'Barrel Type', 
		Callback = function(state)
	...
	barrelDropdown.Visible = false
	
	world_esp_section:AddToggle({
		Text = 'Dropped Items ESP',
		Default = false,
		Callback = function(state)
			droppeditemsesp = state
			if droppeditemsDropdown then
				droppeditemsDropdown.Visible = state
			end
		end
	})
	
	droppeditemsDropdown = world_esp_section:AddDropdown({
		Values = { "All Items", "Weapons", "Tools", "Food", "Medical", "Deployable" }, 
		Default = "",
		Multi = false,
		Text = 'Item Type', 
		Callback = function(state)
	...
	droppeditemsDropdown.Visible = false
	...
	world_esp_section:AddToggle({
		...
		Callback = function(state)
			cratesesp = state
			if cratesDropdown then
				cratesDropdown.Visible = state
			end
		end
	})
	
	cratesDropdown = world_esp_section:AddDropdown({
		Values = { "All Crates", "Food Crate", "Large Crate", "Military Crate", "ExtraMilitaryCrate", "ExtraWoodCrate", "LockedCrate" }, 
		Default = "",
		Multi = false,
		Text = 'Crate Type', 
		Callback = function(state)
	...
	world_esp_section:AddToggle({
		...
		Callback = function(state)
			oresesp = state
			if oresDropdown then
				oresDropdown.Visible = state
			end
		end
	})
	
	oresDropdown = world_esp_section:AddDropdown({
		Values = { "All Ores", "Brimstone Ore", "Iron Ore", "Gold Ore", "Stone Ore" }, 
		Default = "",
		Multi = false,
		Text = 'Ore Type', 
		Callback = function(state)
	...
	world_esp_section:AddToggle({
		...
		Callback = function(state)
			...
			if raidDropdown then
				raidDropdown.Visible = state
			end
		end
	})
	
	raidDropdown = world_esp_section:AddDropdown({
		Values = { "All Raids", "C4", "Rockets", "Satchel" }, 
		Default = "",
		Multi = false,
		Text = 'Raid Type', 
		Callback = function(state)
	...
	world_esp_section:AddSeperator("World ESP Settings")
	...
	WorldSection:AddSeperator("Settings")
	...
	misc_section:AddDropdown({
		Values = { "1", "2", "3", "4", "5", "2", "3", "4", "5", "2", "3", "4", "5", "2", "3", "4", "5", "5", "2", "3", "4", "5", "5", "2", "3", "4", "5", "5", "2", "3", "4", "5" }, 
		Default = "",
		Multi = false,
	
		Text = 'Tracer Textures', 
